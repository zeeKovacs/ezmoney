package gambling;

public class GamblingSim {

    public static void Main(Strnig[] args) {

        //Main : Starts the application. Able to configure the round of the simulations to run by taking an argument from the command line. It has the following methods:

        // public generateHistoricalDatas(int round)
            //returns a HistoricalDatas instance that contains the simulation result

        //main(String[] args) , the main entry point of the application.

    }    
}